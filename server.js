...
app.listen(PORT, () => {
  console.log(`🚀 Webhook is live on port ${PORT}`);
...
app.get("/webhook", (req, res) => {
  ...
  console.error(
    " Webhook Verification Failed! Kindly check all the credentials"
  );
  res.sendStatus(403);
});
...
app.post("/webhook", async (req, res) => {
  try {
    ...
    if (message.type === "interactive") {
      ...
      console.log(
        `🟢 Interactive Message - Type: ${type} | Selected ID: ${selectedId}`
      );
    ...
    if (message.text) {
      ...
      if (greetings.some((g) => msgText.includes(g))) {
        ...
        await sendMessage(
          phone_number_id,
          from,
          `🙏 *Thank you for choosing Bluebex Software!*\nLet us know if you need anything else.`
        );
        await sendMessage(phone_number_id, from, contactMessage());
      } else {
        await sendGreetingListTemplate(phone_number_id, from, profileName);
...
async function handleServiceSelection(phone_number_id, recipient, id) {
  ...
  const buttons = [
    {
      type: "reply",
      reply: { id: `${id.split("_")[0]}_learn_more`, title: "📖 Know More" },
    },
            ...
            action: {
                ...
                sections: [
                {
                    ...
                    rows: [
                    {
                        ...
                        description: "Websites, SEO, Portals",
                    },
                    {
                        id: "app_dev",
                    ...
                    },
                    {
                        id: "AI&ML",
                        title: "🤖 AI & ML",
                        description: "Chatbots, Forecasting, OpenAI",
                    },
                    ],
                },
                ],
